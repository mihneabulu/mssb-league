...
export type Batting = {
  gp: number; ab: number; h: number; '1b': number; '2b': number; '3b': number;
  hr: number; rbi: number; bb: number; hbp: number; sf: number; so: number;
  sb: number; starHits: number; pa: number; tb: number;
  avg: number; obp: number; slg: number; ops: number;
};
export type Pitching = {
  gp: number; outs: number; bf: number; r: number; er: number; h: number;
  hr: number; bb: number; hbp: number; so: number; pitches: number;
  wasPitcher: number; ip: number; era: number; whip: number; k9: number;
};
export type RosterChar = {
  charId: number; name: string; captain: boolean; portrait: string;
};
export type Team = {
  name: string; slug: string; color: string; captainCharId: number;
  captainPortrait: string; stadium: string; roster: RosterChar[];
  record: { w: number; l: number; t: number; gp: number; rf: number; ra: number; diff: number; pct: number };
  batting: Batting; pitching: Pitching; rank: number;
};
export type BoxPlayer = { charId: number; name: string; captain: boolean; batting: Batting; pitching: Pitching };
export type Game = {
  gameId: string; week: number | null; date: number; dateISO: string;
  stadiumId: number; innings: number; away: string; home: string;
  awayScore: number; homeScore: number; winner: string | null;
  boxscore: { away: BoxPlayer[]; home: BoxPlayer[] };
};
export type CharAgg = { charId: number; name: string; team: string; portrait: string; batting: Batting; pitching: Pitching };
...
export const charName = (charId: number): string => data.characterNames[String(charId)] ?? `#${charId}`;
...
export function findGame(a: string, b: string, afterOf: Set<string>): Game | undefined {
  return data.games.find(
    (g) => !afterOf.has(g.gameId) &&
      ((g.away === a && g.home === b) || (g.away === b && g.home === a)),
  );
}
